                ...
                <div class="row justify-content-center align-items-center vh-100 overflow-auto flex-wrap login-bg1">
                    <div class="col-md-9 mx-auto p-4">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="mb-4">
                                    <h2 class="mb-2">{{ __('Set Up Two-Factor Authentication') }}</h2>
                                    <p class="mb-0 fs-16">{{ __('Scan the QR code with your authenticator app, then enter the 6-digit code to verify.') }}</p>
                                </div>
                                @if($errors->any())
                                ...
                                <div class="mb-4">
                                    <label class="form-label text-muted small">{{ __('Or enter this key manually:') }}</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control font-monospace text-center" value="{{ $secret }}" readonly id="secretKey">
                                        <button class="btn btn-outline-secondary" type="button" onclick="navigator.clipboard.writeText(document.getElementById('secretKey').value)">
                                ...
                                <form method="POST" action="{{ route('2fa.verify-setup') }}">
                                    ...
                                    <button type="submit" class="btn btn-primary w-100 justify-content-center">
                                        <i class="ti ti-shield-check me-1"></i> {{ __('Verify & Enable 2FA') }}
                                    </button>
                                </form>
                                <div class="mt-3 text-center">
                                    <a href="{{ route('settings') }}" class="text-muted">{{ __('Cancel') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
