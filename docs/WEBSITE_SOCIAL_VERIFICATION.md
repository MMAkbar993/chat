...
| **LinkedIn: Profile URL vs profile image** | We store two things: (1) **Profile image** – from LinkedIn OpenID (`picture` claim), stored in `social_accounts.oauth_data.avatar`. (2) **Public profile URL** – we try to build `https://www.linkedin.com/in/{vanityName}`. OpenID often does *not* return `vanityName`; we then try a follow-up call to `GET https://api.linkedin.com/v2/me` with the OAuth token to get `vanityName`. That call may fail if the token does not have `r_basicprofile` (e.g. OpenID-only tokens). If we cannot get `vanityName`, we store the generic `https://www.linkedin.com/` so the link still appears. Users can later add their exact profile URL in profile/settings if they want. |
