...
| **LinkedIn: Profile URL vs profile image** | We store two things: (1) **Profile image** – from LinkedIn OpenID (`picture` claim), stored in `social_accounts.oauth_data.avatar`. (2) **Public profile URL** – we try to build `https://www.linkedin.com/in/{vanityName}`. OpenID often does *not* return `vanityName`; we then try a follow-up call to `GET https://api.linkedin.com/v2/me` with the OAuth token to get `vanityName`. That call may fail if the token does not have `r_basicprofile` (e.g. OpenID-only tokens). If we cannot get `vanityName`, we do **not** overwrite the user’s LinkedIn URL in profile; they can enter their full URL (e.g. `https://www.linkedin.com/in/username`) in **Settings → Profile / Social** and save. That value is stored in `user_details.linkedin` and synced to `social_accounts.profile_url`, so the link will show correctly everywhere. |
