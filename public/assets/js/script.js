...
(function ($) {
	...
	if ($('.datetimepicker').length > 0) {
        ...
        $('.datetimepicker').datetimepicker({
            ...
            maxDate: yesterday, // Exclude today and future dates
            defaultDate: null
        }).val('');
    }
		
	// 		if ($('.datetimepicker').length > 0) {
	// 	const today = moment(); // Use Moment.js to handle dates
	// 	const yesterday = moment().subtract(1, 'days'); // Yesterday's date
	...
	// 			previous: 'fas fa-angle-left'
	// 		},
	// 		maxDate: yesterday, // Prevent selection beyond yesterday
	// 		defaultDate: null, // Keep no pre-selected date in the input field
	// 	}).on('dp.show', function () {
	// 		// Highlight today's date manually after the picker opens
	// 		$('.datetimepicker .bootstrap-datetimepicker-widget .day')
	// 			.filter(function () {
	// 				return $(this).text() === today.date().toString(); // Match today's day number
	// 			})
	// 			.addClass('highlight-today'); // Add a custom class for visual highlighting
	// 	});
	// }
	//New Group
	...
	inputs.forEach(input => {
		input.addEventListener('input', function() {
			if (input.value.trim() !== '') {
				...
				// If the input is empty, set it to invalid
